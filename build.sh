...

{
    ...
    node ./cleanup-issues-json.js ./asset/json/displayed-issues.json
    node ./cleanup-issues-json.js ./asset/json/maintenances.json
    ...
    for row in $(cat ./asset/json/issues.json | jq -r '.[] | @base64'); do
        ...
        if [[ $comments != 0 ]]; then
            ...
            node ./cleanup-comments-json.js ./asset/json/comments/$(_jq '.number')-comments.json
        fi
    done
}
